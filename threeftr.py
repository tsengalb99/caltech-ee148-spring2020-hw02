...
def compute_convolution(I, T, stride=1):
    ...
    ts1 = T.shape[0] - 1
    ts2 = T.shape[1] - 1
    tmp = np.zeros((ts1 + I.shape[0], ts2 + I.shape[1], 3))
    tmp[int(ts1/2):int(ts1/2) + I.shape[0], int(ts2/2):int(ts2/2) + I.shape[1]] = I
    I = tmp
...
def detect_red_light_mf(I, fname, ftr1, ftr2, ftr3):
    h1 = compute_convolution(I, ftr1)
    h2 = compute_convolution(I, ftr2)
    h3 = compute_convolution(I, ftr3)
    hmr = np.array(h1) + np.array(h2) + np.array(h3)
    heatmap = normalize(hmr) * 255
    tmp = Image.fromarray(heatmap.astype(np.uint8))
    tmp.save(fname)
    return predict_boxes(hmr, I)


data_path = '../data/RedLights2011_Medium'
split_path = '../data/hw02_splits'
file_names_train = np.load(os.path.join(split_path,'file_names_test.npy'))
file_names_test = np.load(os.path.join(split_path,'file_names_test.npy'))

# set a path for saving predictions:
preds_path = '../data/hw02_predstt'
...
ftr1 = ftr.resize((5, 10))
ftr1 = np.array(ftr1)
ftr2 = ftr.resize((8, 16))
ftr2 = np.array(ftr2)
ftr3 = ftr.resize((11, 22))
ftr3 = np.array(ftr3)
...
def fnametmt(fname):
    ...
    boxes = detect_red_light_mf(np.asarray(I), fname, ftr1, ftr2, ftr3)
    for rc in boxes:
...
with open(os.path.join(preds_path,'preds_traintt.json'),'w') as f:
